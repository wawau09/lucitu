...
Future<void> main() async {
  ...
  // Initialize Naver Map for native mobile platforms to prevent crash
  if (!kIsWeb) {
    try {
...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: AuthCheckScreen(onThemeToggle: toggleTheme, isDark: isDark),
    );
  }
}

/// Dynamic D-Day calculations
String getDDayString(DateTime targetDate) {
...
class AuthCheckScreen extends StatefulWidget {
  ...
  final bool isDark;
  const AuthCheckScreen({super.key, required this.onThemeToggle, required this.isDark});
...
class _AuthCheckScreenState extends State<AuthCheckScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_currentUser != null || _offlineBypass) {
...
/// Premium iOS Style Login Screen
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _signInAnonymously() async {
    ...
    try {
      ...
      debugPrint('Anonymous sign-in failed, bypassing: $e');
      widget.onGuestLogin();
    } finally {
      if (mounted) {
  ...
  Future<void> _signInWithGoogle() async {
    ...
    try {
      ...
      debugPrint('Google Sign-In failed, bypassing: $e');
      widget.onGuestLogin();
    } finally {
      if (mounted) {
        setState(() => _isLoading = false);
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: widget.isDark
                    ? [const Color(0xFF000000), const Color(0xFF1A1A24)]
                    : [const Color(0xFFE5E9F0), const Color(0xFFECEFF4)],
                      ...
                      SizedBox(
                        ...
                        child: CupertinoButton(
                          ...
                          child: Row(
                            ...
                            children: [
                              const Icon(CupertinoIcons.circle_grid_hex, color: Colors.white, size: 20),
...
/// All-in-One Cupertino Planner Dashboard
...
class _PlannerDashboardState extends State<PlannerDashboard> with SingleTickerProviderStateMixin {
  ...
  /// Initial DB Reset & Configuration
  Future<void> _initializePlan() async {
    ...
    try {
      ...
      // Check if "우리들의 감성 제주" exists
      final existingPlans = await Supabase.instance.client
          .from('plans')
          .select()
          .eq('owner_id', user.id)
          .eq('name', '우리들의 감성 제주');
      ...
      if (existingPlans.isEmpty) {
        // DB는 내가 원래 쓰던거랑 연동해서 해줘 기존 plan은 지워줘
        // Clear all existing plans owned by this user
        await Supabase.instance.client
            .from('plans')
            .delete()
            .eq('owner_id', user.id);

        // Create the new plan locked to 2026.07.12
        final newPlan = await Supabase.instance.client.from('plans').insert({
          'owner_id': user.id,
          'name': '우리들의 감성 제주',
          'plan_date': '2026-07-12',
          'plan_code': 'PL-JEJU-${DateTime.now().millisecondsSinceEpoch % 10000}',
        }).select().single();

        _planId = newPlan['id'];

        // Populate initial travelers (collaborators)
        final defaultCollabs = [
          {'name': '민수', 'emoji': '🐶', 'role': '드라이버 🚗', 'email': 'minsu@traveler.com'},
          {'name': '수연', 'emoji': '🐱', 'role': '총무 💰', 'email': 'suyeon@traveler.com'},
          {'name': '지현', 'emoji': '🦊', 'role': '촬영기사 📸', 'email': 'jihyun@traveler.com'},
        ];

        for (var c in defaultCollabs) {
          final roleStr = '${c['name']}|${c['emoji']}|${c['role']}';
          await Supabase.instance.client.from('plan_collaborators').insert({
            'plan_id': _planId,
        ...
        for (var e in defaultEvents) {
          ...
          await Supabase.instance.client.from('plan_items').insert({
            'plan_id': _planId,
            'title': e['title'],
        ...
        // Populate initial checklist items
        final defaultChecklist = [
          {'title': '신분증 & 운전면허증 🪪', 'checked': true},
          {'title': '카메라 & 보조배터리 🔋', 'checked': true},
          {'title': '편안한 운동화 👟', 'checked': false},
          {'title': '선크림 & 선글라스 🕶️', 'checked': false},
        ];

        for (var idx = 0; idx < defaultChecklist.length; idx++) {
          final item = defaultChecklist[idx];
          final noteMap = {
            'category': 'checklist',
        ...
        _planId = existingPlans.first['id'];
      }

      await _fetchPlanData();
    } catch (e) {
      debugPrint('Error resetting DB/loading: $e');
      _loadLocalMockData();
    } finally {
      setState(() => _isLoading = false);
    }
  ...
  Future<void> _addParticipant(String name, String emoji, String role) async {
    ...
    final currentPlanId = _planId;
    if (!_isOffline && currentPlanId != null) {
      try {
  ...
  Future<void> _deleteParticipant(Participant p) async {
    final currentPlanId = _planId;
    if (!_isOffline && currentPlanId != null && !p.id.startsWith('mock_') && !p.id.startsWith('local_')) {
      try {
        await Supabase.instance.client
            .from('plan_collaborators')
            .delete()
  ...
  Future<void> _updateTravelEvent(TravelEvent updatedEvent) async {
    final currentPlanId = _planId;
    if (!_isOffline && currentPlanId != null && !updatedEvent.id.startsWith('mock_') && !updatedEvent.id.startsWith('local_')) {
      try {
        final noteMap = {
  ...
  Future<void> _deleteTravelEvent(String eventId) async {
    final currentPlanId = _planId;
    if (!_isOffline && currentPlanId != null && !eventId.startsWith('mock_') && !eventId.startsWith('local_')) {
      try {
        await Supabase.instance.client
            .from('plan_items')
            .delete()
  ...
  Future<void> _toggleChecklistItem(ChecklistItem item) async {
    ...
    final currentPlanId = _planId;
    if (!_isOffline && currentPlanId != null && !item.id.startsWith('mock_') && !item.id.startsWith('local_')) {
      try {
        final noteMap = {
          'category': 'checklist',
          'checked': newChecked,
  ...
  Future<void> _deleteChecklistItem(String id) async {
    final currentPlanId = _planId;
    if (!_isOffline && currentPlanId != null && !id.startsWith('mock_') && !id.startsWith('local_')) {
      try {
          ...
          : SafeArea(
              ...
              child: Center(
                child: Container(
                  ...
                  color: widget.isDark ? Colors.black : const Color(0xFFF2F2F7),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.stretch,
                    children: [
                        ...
                        child: RefreshIndicator(
                          ...
                          child: ListView(
                            ...
                            children: [
                              ...
                              _buildParticipantsWidget(cardBg),
                              const SizedBox(height: 16),

                              // 4. Expense Tracker with custom doughnut chart
                              _buildExpenseWidget(cardBg),
                              const SizedBox(height: 16),

                              // 5. Timeline header filters
                              _buildTimelineFilterWidget(),
                              const SizedBox(height: 8),

                              // Chronological Timeline items list
                              filteredEvents.isEmpty
                                  ? _buildEmptyTimelineState(cardBg)
                                  : ListView.builder(
                                      shrinkWrap: true,
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(
                    color: widget.isDark ? const Color(0xFF2E2E32) : const Color(0xFFF2F2F7),
                    shape: BoxShape.circle,
                  ),
                  child: Icon(
                    widget.isDark ? CupertinoIcons.sun_max_fill : CupertinoIcons.moon_fill,
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Icon(
                    ...
                    color: widget.isDark ? Colors.red[400] : Colors.red[700],
                    size: 18,
                  ),
                ),
              ),
  ...
  Widget _buildTripOverviewCard() {
    ...
    final dDayStr = getDDayString(targetDate);
  ...
  /// 3. Participants Widget
  Widget _buildParticipantsWidget(Color cardBg) {
    return Container(
      decoration: BoxDecoration(
        color: cardBg,
        borderRadius: BorderRadius.circular(20),
  ...
  Widget _buildExpenseWidget(Color cardBg) {
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text(
                '${(progress * 100).toStringAsFixed(0)}% (${formatKRW(budget)} 기준)',
                style: GoogleFonts.outfit(
                  fontSize: 12,
                  fontWeight: FontWeight.bold,
          ...
          ClipRRect(
            ...
            child: SizedBox(
              ...
              child: LinearProgressIndicator(
                ...
                backgroundColor: widget.isDark ? const Color(0xFF2C2C2E) : const Color(0xFFE5E5EA),
  ...
  Widget _buildTimelineFilterWidget() {
    ...
    return Column(
      ...
      children: [
        ...
        CupertinoSearchTextField(
          ...
          placeholderStyle: TextStyle(color: widget.isDark ? Colors.white38 : Colors.black38),
                ...
                child: AnimatedContainer(
                  ...
                  child: Center(
                    child: Text(
                      cat,
                      style: GoogleFonts.notoSansKr(
                        color: isSel ? Colors.white : (widget.isDark ? Colors.white54 : Colors.black54),
                        fontSize: 12,
                        fontWeight: isSel ? FontWeight.bold : FontWeight.normal,
                      ),
                    ),
  ...
  Widget _buildEmptyTimelineState(Color cardBg) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(32),
      decoration: BoxDecoration(
      ...
      child: Column(
        children: [
          Icon(CupertinoIcons.calendar_badge_minus, size: 40, color: widget.isDark ? Colors.white24 : Colors.black26),
          const SizedBox(height: 12),
          Text(
            '해당되는 일정이 없습니다.',
            style: GoogleFonts.notoSansKr(
            ...
            child: Column(
              children: [
                Expanded(
                  child: Container(
                    width: 2,
                    color: isFirst ? Colors.transparent : (widget.isDark ? const Color(0xFF2C2C2E) : const Color(0xFFE5E5EA)),
                  ),
                ),
                GestureDetector(
                ...
                Expanded(
                  child: Container(
                    width: 2,
                    color: isLast ? Colors.transparent : (widget.isDark ? const Color(0xFF2C2C2E) : const Color(0xFFE5E5EA)),
                  ),
                ),
              ],
            ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        PopupMenuButton<String>(
                          icon: Icon(CupertinoIcons.ellipsis_vertical, size: 14, color: widget.isDark ? Colors.white38 : Colors.black38),
                          padding: EdgeInsets.zero,
                          style: ButtonStyle(
  ...
  Widget _buildChecklistWidget(Color cardBg) {
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                '체크 비율',
                style: GoogleFonts.notoSansKr(fontSize: 11, color: widget.isDark ? Colors.white38 : Colors.black38),
              ),
              Text(
          ...
          ClipRRect(
            ...
            child: SizedBox(
              ...
              child: LinearProgressIndicator(
                value: pct,
                backgroundColor: widget.isDark ? const Color(0xFF2C2C2E) : const Color(0xFFF2F2F7),
          ...
          _checklist.isEmpty
              ? Padding(
                  ...
                  child: Center(
                    child: Text(
                      '준비물이 비어있습니다.',
                      style: GoogleFonts.notoSansKr(fontSize: 12, color: widget.isDark ? Colors.white38 : Colors.black38),
                    ),
                  ),
                )
              : ListView.builder(
                      ...
                      child: Row(
                        children: [
                          ...
                          GestureDetector(
                            ...
                            child: Icon(CupertinoIcons.xmark, size: 14, color: widget.isDark ? Colors.white24 : Colors.black26),
                          ),
                        ],
                      ),
                    );
